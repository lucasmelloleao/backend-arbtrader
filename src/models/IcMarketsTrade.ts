...
export interface IIcMarketsTrade extends mongoose.Document {
...
const IcMarketsTradeSchema = new mongoose.Schema(
  {
    userId: { type: mongoose.Schema.Types.ObjectId, ref: 'User', required: true, index: true },
    strategyId: { type: mongoose.Schema.Types.ObjectId, ref: 'IcMarketsStrategy', required: false, index: true },
...
IcMarketsTradeSchema.index({ strategyId: 1, closedAt: -1 });
IcMarketsTradeSchema.index({ userId: 1, symbol: 1, status: 1 });

export default mongoose.models.IcMarketsTrade || mongoose.model<IIcMarketsTrade>('IcMarketsTrade', IcMarketsTradeSchema);
