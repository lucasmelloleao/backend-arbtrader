...
export async function runMarketMaking(
  ...
  const settingsGlobal = await (PredictionArbSettings as any).findOne().lean().catch(() => null);
  const minProb = Number(strategy.minHighCertaintyProb ?? settingsGlobal?.minHighCertaintyProb ?? PREDICTION_ARB_CONFIG.scan.minHighCertaintyProb ?? 0.95);
